...
from clustercontrast.models.cm import ClusterMemory,HybridMemory
from clustercontrast.trainers import Trainer_Stage1, Trainer_Stage2, Trainer_Stage3
...
from clustercontrast.utils.faiss_rerank import compute_modal_camera_invariant_jaccard_distance,compute_camera_invariant_jaccard_distance
...
def extract_gall_feat(model, gall_loader, ngall, modal):
    pool_dim = 2048
    net = model
    net.eval()
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(gall_loader):
            ...
            feat_fc = net(input, input, extract_modal)
            flip_input = Variable(flip_input.cuda())
            feat_fc_1 = net(flip_input, flip_input, extract_modal)
...
def extract_query_feat(model, query_loader, nquery, modal):
    pool_dim = 2048
    net = model
    net.eval()
...
def main():
    ...
    log_s3_name = 'regdb_s3'

    print('\n **************MPJL_REGDB||2025************** \n')

    subdir = datetime.strftime(datetime.now(), '%Y%m%d-%H%M%S')
    main_worker_stage1(args,log_s1_name,log_s2_name,subdir)

    update_subdir1 = datetime.strftime(datetime.now(), '%Y%m%d-%H%M%S')
    main_worker_stage2(args,log_s1_name,log_s2_name,subdir,update_subdir1)

    update_subdir2 = datetime.strftime(datetime.now(), '%Y%m%d-%H%M%S')
    main_worker_stage3(args,log_s2_name,log_s3_name,update_subdir1,update_subdir2)

    print('\n **************MPJL_REGDB||2025************** \n')

def main_worker_stage1(args, log_s1_name, log_s2_name, subdir):
    ...
    # Trainer
    trainer = Trainer_Stage1(model, net_modal_classifier1)


    memory_instance_IR = HybridMemory(model.module.num_features, len(dataset_ir.train),
                            temp=args.temp, momentum=args.momentum).cuda() ##momentun == 1?
    #################################
    memory_instance_RGB = HybridMemory(model.module.num_features, len(dataset_rgb.train),
                            temp=args.temp, momentum=args.momentum).cuda() ##momentun == 1?
    #################################
    print('==> Initialize instance features in the IR hybrid memory')
    ...
    del cluster_loader_ir,features_ir
    print('==> Initialize instance features in the RGB hybrid memory')
    ...
    del cluster_loader_rgb,features_rgb

    for epoch in range(30):

        with torch.no_grad():

            def exponential_decay(initial_value, total_epochs, current_epoch):
                decay_factor = (2/3) ** (current_epoch / total_epochs)
                current_value = initial_value * decay_factor
                return current_value


            def exponential_decay_kb(initial_value, total_epochs, current_epoch, factor):
                decay_factor = (factor) ** (current_epoch / total_epochs)
                current_value = initial_value * decay_factor
                current_value = np.around(current_value, 0)
                current_value = int(current_value)
                return current_value

            def exponential_decay_k1(initial_value, total_epochs, current_epoch, factor):
                decay_factor = (factor) ** (current_epoch / total_epochs)
                current_value = initial_value * decay_factor
                current_value = np.around(current_value, 0)
                current_value = int(current_value)
            ...
            def exponential_decay_mo(initial_value, total_epochs, current_epoch):
                decay_factor = (3) ** (current_epoch / total_epochs)
                current_value = initial_value * decay_factor
                return current_value


            eps=exponential_decay(0.3,args.epochs,epoch)
            # if epoch == 0:
                # DBSCAN cluster
            ir_eps = eps
            ...
            rerank_dist_rgb = compute_camera_invariant_jaccard_distance(features_rgb, k1=k1, kb=kb, search_option=3,file=sorted(dataset_rgb.train),camera_num=1)
        ...
        # # generate new dataset and calculate cluster centers
        def generate_pseudo_labels(cluster_id, num, dataset):
            labels = []
            outliers = 0
        ...
        memory_rgb = ClusterMemory(model.module.num_features, num_cluster_rgb, temp=args.temp,
                                   momentum=args.momentum, use_hard=args.use_hard).cuda()
        print('momentum: ', memory_ir.momentum)
        ...
        #####################################
        pseudo_labels_ir = generate_pseudo_labels(pseudo_labels_ir, num_cluster_ir, dataset_ir)
        print('pseudo_labels_ir', pseudo_labels_ir)
        print('pseudo_labels_ir.shape', pseudo_labels_ir.shape)
        memory_instance_IR.labels = pseudo_labels_ir.cuda()  # [12936]

        pseudo_labels_rgb = generate_pseudo_labels(pseudo_labels_rgb, num_cluster_rgb, dataset_rgb)
        memory_instance_RGB.labels = pseudo_labels_rgb.cuda()  # [12936]
        print('pseudo_labels_rgb', pseudo_labels_rgb)
        print('pseudo_labels_rgb.shape', pseudo_labels_rgb.shape)

        memory_instance_IR.momentum = args.momentum
        memory_instance_RGB.momentum = args.momentum
        print('ins_momentum:', memory_instance_IR.momentum)

        trainer.memory_instance_ir = memory_instance_IR  # [12936, 2048]]
        trainer.memory_instance_rgb = memory_instance_RGB  # [12936, 2048]]

        train_loader_all_ir = []
        train_loader_all_rgb = []
        ...
        if epoch >= 0:
            ...
            query_feat_fc = extract_query_feat(model, query_loader, nquery, modal='rgb')
            # for trial in range(1):
            ngall = len(gall_label)
            gall_feat_fc = extract_gall_feat(model, gall_loader, ngall, modal='ir')
            # fc feature
...
def main_worker_stage2(args, log_s1_name, log_s2_name, subdir,subdir_update=None):
    if subdir_update is not None:
        logs_dir_root = osp.join(args.logs_dir + '/' + subdir_update + '/' + log_s2_name)
    else:
    ...
    checkpoint = load_checkpoint(
        osp.join('./logs/regdb/' + subdir + '/' + log_s1_name + '/' + str(trial), 'model_best.pth.tar'))
    ...
    # Trainer
    trainer = Trainer_Stage2(model, net_modal_classifier1)
    ...
    del cluster_loader_rgb,features_rgb,cluster_loader_ir,features_ir
    for epoch in range(30):
        if epoch == 0:
            ##############################
            args.test_batch = 128
        ...
        with torch.no_grad():
            ...
            def exponential_decay_increase(initial_value, total_epochs, current_epoch):
                decay_factor = (3/2) ** (current_epoch / total_epochs)
                current_value = initial_value * decay_factor
                return current_value
            ...
            if epoch<=10:
                ...
                eps = exponential_decay(0.3, 10, epoch)
            else:
                eps = exponential_decay_increase(0.2, 40, epoch-10)
            ...
            rerank_dist_ir = compute_camera_invariant_jaccard_distance(features_ir, k1=k1, kb=kb,
                                                      search_option=3,file=sorted(dataset_ir.train),camera_num=1)
            pseudo_labels_ir = cluster_ir.fit_predict(rerank_dist_ir)
            rerank_dist_rgb = compute_camera_invariant_jaccard_distance(features_rgb, k1=k1, kb=kb,
                                                       search_option=3,file=sorted(dataset_rgb.train),camera_num=1)
            pseudo_labels_rgb = cluster_rgb.fit_predict(rerank_dist_rgb)
        ...
        if epoch >= 0:
            ...
            feat_rgb = features_rgb.cuda()
            feat_ir = features_ir.cuda()
            mem_rgb = trainer.memory_rgb.features.clone().cuda()
            mem_ir = trainer.memory_ir.features.clone().cuda()

            pseudo_labels_rgb_np = np.array(pseudo_labels_rgb)
            pseudo_labels_ir_np = np.array(pseudo_labels_ir)

            from collections import defaultdict
            rgb_cluster_to_samples = defaultdict(list)
            ir_cluster_to_samples = defaultdict(list)

            for idx, lbl in enumerate(pseudo_labels_rgb_np):
                if lbl != -1:
                    rgb_cluster_to_samples[lbl].append(idx)
            for idx, lbl in enumerate(pseudo_labels_ir_np):
                if lbl != -1:
                    ir_cluster_to_samples[lbl].append(idx)

            THRESHOLD = 0.5
            LAMBDA = 0.4
            print('THRESHOLD={}, LAMBDA={}'.format(THRESHOLD, LAMBDA))

            # Visible → Infrared
            updated_ir_indices = set()
            for rgb_cid in sorted(rgb_cluster_to_samples.keys()):
                sample_indices = rgb_cluster_to_samples[rgb_cid]
                if len(sample_indices) == 0:
                    continue
                rgb_samples = feat_rgb[sample_indices]

                sim = torch.mm(rgb_samples, mem_ir.t())
                ...
                sim_offset = torch.clamp(sim - THRESHOLD, min=0.0)
                scores = sim_offset.mean(dim=0)

                best_ir_cid = torch.argmax(scores).item()
                best_score = scores[best_ir_cid].item()

                if best_score <= 0:
                    continue

                matched_mask = sim[:, best_ir_cid] >= THRESHOLD
                if matched_mask.sum() == 0:
                    continue
                matched_rgb_feats = rgb_samples[matched_mask]
                avg_matched_rgb = matched_rgb_feats.mean(dim=0)

                trainer.memory_ir.features[best_ir_cid] = \
                    LAMBDA * trainer.memory_ir.features[best_ir_cid] + (1 - LAMBDA) * avg_matched_rgb
                updated_ir_indices.add(best_ir_cid)

            # Infrared → Visible
            updated_rgb_indices = set()
            for ir_cid in sorted(ir_cluster_to_samples.keys()):
                sample_indices = ir_cluster_to_samples[ir_cid]
                if len(sample_indices) == 0:
                    continue
                ir_samples = feat_ir[sample_indices]

                sim = torch.mm(ir_samples, mem_rgb.t())
                sim = torch.clamp(sim, -1, 1)
                ...
                sim_offset = torch.clamp(sim - THRESHOLD, min=0.0)
                scores = sim_offset.mean(dim=0)  # [L]

                best_rgb_cid = torch.argmax(scores).item()
                best_score = scores[best_rgb_cid].item()

                if best_score <= 0:
                    continue

                matched_mask = sim[:, best_rgb_cid] >= THRESHOLD
                if matched_mask.sum() == 0:
                    continue
                matched_ir_feats = ir_samples[matched_mask]
                avg_matched_ir = matched_ir_feats.mean(dim=0)

                trainer.memory_rgb.features[best_rgb_cid] = \
                    LAMBDA * trainer.memory_rgb.features[best_rgb_cid] + (1 - LAMBDA) * avg_matched_ir
                updated_rgb_indices.add(best_rgb_cid)
...
def main_worker_stage3(args, log_s1_name, log_s2_name, subdir,subdir_update=None):
    if subdir_update is not None:
        logs_dir_root = osp.join(args.logs_dir + '/' + subdir_update + '/' + log_s2_name)
    else:
        logs_dir_root = osp.join(args.logs_dir + '/' + subdir + '/' + log_s2_name)
    ...
    ### loading checkpoint
    checkpoint = load_checkpoint(
        osp.join(temp_logs_dir, 'model_best.pth.tar'))
    ...
    # Trainer
    trainer = Trainer_Stage3(model, net_modal_classifier1)
    ...
    memory_instance_RGB = HybridMemory(model.module.num_features, len(dataset_rgb.train),
                            temp=args.temp, momentum=args.momentum).cuda() ##momentun == 1?
    #################################
    memory_instance_ALL = HybridMemory(model.module.num_features, len(dataset_rgb.train+dataset_ir.train),
                            temp=args.temp, momentum=args.momentum).cuda() ##momentun == 1?
    print('==> Initialize instance features in the IR hybrid memory')
    cluster_loader_ir = get_test_loader(dataset_ir, args.height, args.width,
                                        args.batch_size, args.workers,
    ...
    print('==> Initialize instance features in the ALL hybrid memory')
    feature_all = torch.cat([features_rgb, features_ir], dim=0)

    memory_instance_ALL.features = F.normalize(feature_all, dim=1).cuda()

    del cluster_loader_rgb,features_rgb,cluster_loader_ir,features_ir,feature_all
    for epoch in range(60):
        ...
        with torch.no_grad():
            ...
            k1 = exponential_decay_k1(40, args.epochs-1, epoch, 1.5)
            print('k1:', k1)
            k1m = exponential_decay_k1(60, args.epochs-1, epoch, 1.5)
            print('k1m:', k1m)
            kb = exponential_decay_kb(8, args.epochs-1, epoch, 1.5)
            print('kb:',kb)
            kbm = exponential_decay_kb(10, args.epochs-1, epoch, 1.5)
            print('kbm:', kbm)
            ...
            rerank_dist_rgb = compute_camera_invariant_jaccard_distance(features_rgb, k1=k1, kb=kb,
                                                       search_option=3,file=sorted(dataset_rgb.train),camera_num=1)
            pseudo_labels_rgb = cluster_rgb.fit_predict(rerank_dist_rgb)
            rerank_dist_all = compute_modal_camera_invariant_jaccard_distance(feature_all, k1=k1m, kb=kbm, file=sorted(dataset_rgb.train)+sorted(dataset_ir.train),
                                           search_option=3,camera_num=1)
        ...
        def generate_pseudo_labels_all(cluster_id, num, dataset):
            labels = []
            outliers = 0
            for i, ((fname, _, cid), id) in enumerate(zip(dataset, cluster_id)):
                if id!=-1:
                    labels.append(id)
                else:
                    labels.append(num+outliers)
        ...
        cluster_features_ir = generate_cluster_features(pseudo_labels_ir, features_ir)
        cluster_features_rgb = generate_cluster_features(pseudo_labels_rgb, features_rgb)
        cluster_features_all = generate_cluster_features(pseudo_labels_all,feature_all)
        ...
        pseudo_labeled_dataset_all_instance_ir = []
        pseudo_labeled_dataset_all_instance_rgb = []
        all_label = []
        ...
        for i, ((fname, _, cid), label) in enumerate(
            ...
            if label !=-1 and 'ir_modify' in fname:
                pseudo_labeled_dataset_all_instance_ir.append((fname, label.item(), cid))

            elif label != -1 and 'rgb_modify' in fname:
                pseudo_labeled_dataset_all_instance_rgb.append((fname, label.item(), cid))
        ...
        pseudo_labels_all = generate_pseudo_labels_all(pseudo_labels_all, num_cluster_all, sorted(dataset_rgb.train) + sorted(dataset_ir.train))
        memory_instance_ALL.labels = pseudo_labels_all.cuda()

        memory_instance_IR.momentum = args.momentum
        memory_instance_RGB.momentum = args.momentum
        memory_instance_ALL.momentum = args.momentum
        print('ins_momentum:', memory_instance_IR.momentum)

        trainer.memory_instance_ir = memory_instance_IR  # [12936, 2048]]
        trainer.memory_instance_rgb = memory_instance_RGB  # [12936, 2048]]
        trainer.memory_instance_all = memory_instance_ALL  # [12936, 2048]]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, 'logs/regdb'))
